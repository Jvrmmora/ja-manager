...
const AttendanceHistory: React.FC<AttendanceHistoryProps> = ({
  ...
  if (error) {
    return (
      <div className={`text-center py-8 ${className}`}>
        <XCircleIcon
          className={`w-12 h-12 mx-auto mb-4 ${isDark ? 'text-red-400' : 'text-red-500'}`}
        />
    ...
    <div className={`${className}`}>
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {!compact && data.stats && (
            <div
              className={`text-right text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {data.stats.todayAttendance && (
                  ...
                  >
                    Registrada a las{' '}
                    {formatTime(data.stats.todayAttendance.scannedAt)}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="space-y-3">
            {data.attendances.map((attendance: any, index: number) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <CheckCircleIcon
                    className={`w-5 h-5 ${isDark ? 'text-green-400' : 'text-green-500'}`}
                  />
                  <div>
                    <p
        ...
        {!compact && data.pagination && data.pagination.totalPages > 1 && (
          <div className="flex justify-center gap-2 mt-6">
            ...
            <span
              className={`px-3 py-2 text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}
            >
              Página {data.pagination.currentPage} de{' '}
        ...
        {compact && data.stats && (
          ...
          >
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <p
                  className={`text-2xl font-bold ${isDark ? 'text-blue-400' : 'text-blue-500'}`}
                >
              ...
              <div>
                <p
                  className={`text-2xl font-bold ${isDark ? 'text-green-400' : 'text-green-500'}`}
                >
