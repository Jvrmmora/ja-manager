...
interface QRScannerOptimizedProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (data: any) => void;
}

const QRScannerOptimized: React.FC<QRScannerOptimizedProps> = ({
  isOpen,
  onClose,
  onSuccess
}) => {
  ...
  const handleScanResult = async (data: string) => {
    if (!isScanning || isProcessing) {
      console.log('🚫 Ignorando scan - ya procesando o no escaneando');
      return; // Prevenir múltiples scans
    }
    ...
    try {
      ...
      setTimeout(() => {
        if (scannerRef.current && !isProcessing) {
          try {
