...
const processExcelDate = (dateValue: any): Date => {
  ...
  console.warn(
    `No se pudo parsear la fecha: ${dateValue}, usando fecha actual`
  );
  return DateTime.now().toJSDate();
};
    ...
    for (let i = 0; i < data.length; i++) {
      try {
        ...
        if (normalizedRow.fullName) {
          fullName =
            ...
            (normalizedRow.lastName
              ? ` ${normalizedRow.lastName.toString().trim()}`
              : '');
        }
        ...
        if (normalizedRow.phone || normalizedRow.celular) {
          phone = (normalizedRow.phone || normalizedRow.celular)
            .toString()
            .trim();
        ...
        let birthday: Date;
        const birthdayValue =
          normalizedRow.birthday || normalizedRow.fecha_cumpleanos;
        ...
        if (normalizedRow.gender) {
          const genderStr = normalizedRow.gender
            .toString()
            .toLowerCase()
            .trim();
          if (
        ...
        const youngData = {
          ...
          group: (() => {
            ...
            results.warnings.push(
              `Fila ${i + 1}: valor de grupo inválido (${g}), se ignorará`
            );
            return undefined;
          })(),
          skills: [],
        ...
        if (!young) {
          ...
          if (youngData.email) {
            const existingByEmail = await Young.findOne({
              email: youngData.email,
            });
            if (existingByEmail) {
              results.errors.push({
                row: i + 1,
          ...
          if (existingByName) {
            results.warnings.push(
              `Fila ${i + 1}: ${fullName} ya existe en la base de datos`
            );
            continue;
          }
